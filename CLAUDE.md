...
- [scripts/extract-urban-areas.js](scripts/extract-urban-areas.js) — seed → urban-polygon matching by **nearest-seed ownership + adjacency flood-fill** (`GAP_MAX = 8 km`, `REACH_MAX = 45 km`); reunites water-split fragments; partitions shared blobs along perpendicular bisectors via a martinez boolean (`polygon-clipping` devDependency — a concave blob's Voronoi cell can be disconnected, and each lobe must stay its own ring; a Sutherland-Hodgman clip bridges lobes with phantom edges); 2-opt uncrossing after ring decimation; `display: false` shadow seeds; prints a per-city report.
